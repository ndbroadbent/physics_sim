...
//! Fitness = number of correct output bits across 4 test cases (max 8).
...
const MAX_FITNESS: usize = 8; // 4 tests * 2 outputs
...
impl GpuContext {
    async fn new() -> Self {
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Half Adder Evaluator Shader"),
            source: wgpu::ShaderSource::Wgsl(Cow::Borrowed(include_str!(
                "../halfadder_evaluator.wgsl"
            ))),
        });
...
const OUTPUT_SUM: usize = 12;
const OUTPUT_CARRY: usize = 13;

fn print_genome(genome: &Genome) {
    println!("\n--- Best Organism (4x4) ---");
    println!("Inputs: A(0,0), B(1,0)  Outputs: Sum(0,3), Carry(1,3)\n");
    let op_names = [
    ...
    for y in 0..GRID_DIM {
        for x in 0..GRID_DIM {
            ...
            let io = if idx == INPUT_A || idx == INPUT_B {
                "I"
            } else if idx == OUTPUT_SUM || idx == OUTPUT_CARRY {
                "O"
            } else {
                " "
            };
...
fn main() {
    println!("========================================");
    println!("   EVOLVE HALF ADDER");
    println!("   Input: A, B");
    println!("   Output: Sum=XOR, Carry=AND");
    println!("   Fitness: correct bits / 8");
    println!("========================================\n");
