//! Evolve a 2-bit Adder
//!
//! A simpler evolution problem: evolve a circuit that adds two 2-bit numbers.
//! Input: 4 bits (A1 A0 B1 B0)
//! Output: 3 bits (S2 S1 S0) = A + B
//!
//! Uses an 8x8 grid (64 cells) - much simpler than the 16x16 quine problem.
//! Fitness = number of correct output bits across 16 test cases (max 48).
...
const POPULATION_SIZE: usize = 50_000;
const GENOME_SIZE: usize = 64; // 8x8
const GRID_DIM: usize = 8;
const LEARNING_RATE: f32 = 0.05;  // Slower learning for more exploration
const EXPLORATION_RATE: f32 = 0.15; // Probability of random sample instead of from distribution
const MAX_FITNESS: usize = 48; // 16 tests * 3 bits

// Include jump wire ops in mutation
...
struct Cell {
    ...
    dir_a: u32,
    dir_b: u32,
    flags: u32, // Unused - we use fixed I/O positions
}
...
impl Genome {
    fn new_random(rng: &mut impl Rng) -> Self {
        ...
        for _ in 0..GENOME_SIZE {
            cells.push(Cell {
                op: rng.gen_range(0..NUM_OPS as u32),
                dir_a: rng.gen_range(0..8),
                dir_b: rng.gen_range(0..8),
                flags: 0, // Unused - we use fixed I/O positions in shader
            });
        }
        Genome { cells }
    }
...
impl ProbMatrix {
    ...
    fn sample(&self, rng: &mut impl Rng) -> Genome {
        ...
        for i in 0..GENOME_SIZE {
            // With EXPLORATION_RATE probability, sample randomly instead of from distribution
...
impl GpuContext {
    async fn new() -> Self {
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Adder Evaluator Shader"),
            source: wgpu::ShaderSource::Wgsl(Cow::Borrowed(include_str!(
                "../adder_evaluator.wgsl"
            ))),
        });
...
const INPUT_CELLS: [usize; 4] = [0, 1, 2, 3];       // Top row: A0, A1, B0, B1
const OUTPUT_CELLS: [usize; 3] = [56, 57, 58];      // Bottom row: S0, S1, S2

fn print_genome(genome: &Genome) {
    println!("\n--- Best Organism (8x8) ---");
    println!("Inputs: cells 0-3 (top row), Outputs: cells 56-58 (bottom row)\n");
    let op_names = [
    ...
    for y in 0..GRID_DIM {
        for x in 0..GRID_DIM {
            ...
            let is_input = INPUT_CELLS.contains(&idx);
            let is_output = OUTPUT_CELLS.contains(&idx);
            let io = match (is_input, is_output) {
                (true, true) => "IO",
                (true, false) => "I ",
                (false, true) => " O",
                (false, false) => "  ",
...
fn main() {
    println!("========================================");
    println!("   EVOLVE 2-BIT ADDER");
    println!("   Input: A(2b) + B(2b) -> Sum(3b)");
    println!("   Fitness: correct bits / 48");
    println!("========================================\n");
    ...
    for gen in 0..10000 {
        ...
        // Update from top 1% of population
        let top_count = (POPULATION_SIZE / 100).max(50);
        ...
        if generations_since_improvement > 2000 {
            println!("\nStalled for 2000 generations, stopping...");
            break;
        }
    }

    println!("\nFinal best fitness: {}/{} ({:.1}%)",
