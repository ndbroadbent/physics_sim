...
//!
//! Uses a 6x6 grid (36 cells).
//! Fitness = number of correct output bits across 8 test cases (max 16).
...
const POPULATION_SIZE: usize = 20_000;
const GENOME_SIZE: usize = 36; // 6x6
const GRID_DIM: usize = 6;
const LEARNING_RATE: f32 = 0.08;
const EXPLORATION_RATE: f32 = 0.12;
const MAX_FITNESS: usize = 16; // 8 tests * 2 outputs
...
impl GpuContext {
    async fn new() -> Self {
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Full Adder Evaluator Shader"),
            source: wgpu::ShaderSource::Wgsl(Cow::Borrowed(include_str!(
                "../fulladder_evaluator.wgsl"
            ))),
        });
...
const INPUT_CIN: usize = 2;
const OUTPUT_SUM: usize = 30;
const OUTPUT_COUT: usize = 31;

fn print_genome(genome: &Genome) {
    println!("\n--- Best Organism (6x6) ---");
    println!("Inputs: A(0,0), B(1,0), Cin(2,0)");
    println!("Outputs: Sum(0,5), Cout(1,5)\n");
    let op_names = [
    ...
    for y in 0..GRID_DIM {
        for x in 0..GRID_DIM {
            ...
            let io = if idx == INPUT_A || idx == INPUT_B || idx == INPUT_CIN {
                "I"
            } else if idx == OUTPUT_SUM || idx == OUTPUT_COUT {
                "O"
            } else {
                " "
            };
...
fn main() {
    println!("========================================");
    println!("   EVOLVE FULL ADDER");
    println!("   Input: A, B, Cin");
    println!("   Output: Sum, Cout");
    println!("   Fitness: correct bits / 16");
    println!("========================================\n");
    ...
    for gen in 0..20000 {
        ...
        if generations_since_improvement > 3000 {
            println!("\nStalled for 3000 generations, stopping...");
            break;
        }
    }
