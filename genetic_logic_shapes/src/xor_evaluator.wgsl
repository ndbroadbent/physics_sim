// XOR Gate Evaluator
//
// The simplest possible evolution test: learn XOR.
// Input: 2 bits (A, B)
...
// Output at bottom row: cell 12 (result)
const INPUT_A: u32 = 0u;   // A at (0,0)
const INPUT_B: u32 = 1u;   // B at (1,0)
const OUTPUT: u32 = 12u;   // Result at (0,3)
...
fn main(
    ...
    for (var test = 0u; test < TEST_CASES; test++) {
        ...
        // Check output
        if (cell_idx == 0u) {
            let output_val = grid_a[OUTPUT].state & 1u;
    ...
    // Store result: total correct (max = 4)
    if (cell_idx == 0u) {
